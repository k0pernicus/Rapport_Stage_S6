...
\newline
Selon mon tuteur d'entreprise, il fallait juste modifier la 1ère partie de l'algorithme précédent, de sorte à prendre les 3 plus grandes arêtes formant un triangle, dont la somme des côtés sera la longueur maximale de toutes les autres représentations triangulaires possibles, selon les données. Ainsi, chaque noeud du triangle est une source d'une arête, mais aussi une cible d'une autre.
\newline
...
Auparavant, nous utilisions comme distances les pourcentages d'identité - qui sont une approximation de différences entre deux clônes, basés sur des séquences de nucléotides de différentes tailles.
\newline
Afin de pouvoir obtenir une distance beaucoup plus précise, j'ai eu l'idée de comparer des séquences de nucléotides de tailles fixes, présentes dans les jeux de données: les \textit{windows}.

\section{Utilisation des \textit{windows}}

\textcolor{red}{Qu'est-ce que les Windows?}
\textcolor{red}{Pourquoi les utiliser?}

\section{Les différents coûts utilisés}

\textcolor{red}{La programmation dynamique}
\textcolor{red}{Quels sont les coûts, pourquoi les avoir choisi?}

\section{Le coût utilisé}

\textcolor{red}{Pour ce coût-ci?}
\textcolor{red}{Apports aux graphes}
